...
void lcdInitialize(uint8_t dataBitMode, uint8_t numLines, uint8_t charFont) {
    ...
    if(dataBitMode == 0b00000000) {
        lcdWriteNibble(0, 0b00000000 >> 4);
    } else {
        lcdWriteByte(0, 0b00010000);
    }
    _delay((unsigned long)((50)*(16000000/4000000.0)));
    ...
    lcdClearDisplay();
    lcdDisplayControl(0b00000100, 0b00000000, 0b00000000);
    lcdEntryMode(0b00000010, 0b00000000);
}

void lcdWriteNibble(uint8_t regType, char nibble) {
    LATEbits.LATE2 = 0;
    LATEbits.LATE0 = regType;
    LATEbits.LATE1 = 0;

    LATAbits.LATA0 = (nibble & 0x01);
    LATAbits.LATA1 = (nibble & 0x02) >> 1;
    LATAbits.LATA2 = (nibble & 0x04) >> 2;
    LATAbits.LATA3 = (nibble & 0x08) >> 3;
...
void lcdSetCursor(uint8_t row, uint8_t col) {
    if(row == 0) {
        lcdWriteByte(0, 0b10000000 + col);
    } else if(row == 1) {
        lcdWriteByte(0, 0b11000000 + col);
    }
    _delay((unsigned long)((50)*(16000000/4000000.0)));
}

void lcdFunctionSet(uint8_t dataBitMode, uint8_t displayLines, uint8_t charFont) {
    uint8_t cmd = 0b00100000 | dataBitMode | displayLines | charFont;
    lcdWriteByte(0, cmd);
    _delay((unsigned long)((50)*(16000000/4000.0)));
}
...
void lcdEntryMode(uint8_t moveDir, uint8_t cursorMovement) {
    uint8_t cmd = 0b00000100 | moveDir | cursorMovement;
    lcdWriteByte(0, cmd);
    _delay((unsigned long)((50)*(16000000/4000000.0)));
}
void lcdDisplayControl(uint8_t display, uint8_t cursor, uint8_t blink) {
    uint8_t cmd = 0b00001000 | display | cursor | blink;
    lcdWriteByte(0, cmd);
    _delay((unsigned long)((50)*(16000000/4000000.0)));
}
void lcdCursorDisplayShift(uint8_t cursor, uint8_t displayShift) {
    uint8_t cmd = 0b00010000 | cursor | displayShift;
    lcdWriteByte(0, cmd);
    _delay((unsigned long)((50)*(16000000/4000000.0)));
}
